...
describe('StreamingMessageParser', () => {
  ...
  describe('no artifacts', () => {
    ...
    ])('should correctly parse chunks and strip out rill artifacts (%#)', (input, expected) => {
  ...
  describe('invalid or incomplete artifacts', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      ...
      ['Foo bar <bol', 'Foo bar '],
      ['Foo bar <rill', 'Foo bar '],
      ['Foo bar <rilla', 'Foo bar <rilla'],
      ['Foo bar <rillA', 'Foo bar '],
      ['Foo bar <rillArtifacs></rillArtifact>', 'Foo bar <rillArtifacs></rillArtifact>'],
      ['Before <oltArtfiact>foo</rillArtifact> After', 'Before <oltArtfiact>foo</rillArtifact> After'],
      ['Before <rillArtifactt>foo</rillArtifact> After', 'Before <rillArtifactt>foo</rillArtifact> After'],
    ])('should correctly parse chunks and strip out rill artifacts (%#)', (input, expected) => {
      runTest(input, expected);
    });
  });
  ...
  describe('valid artifacts without actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Some text before <rillArtifact title="Some title" id="artifact_1">foo bar</rillArtifact> Some more text',
        {
      ...
      [
        ['Some text before <rillArti', 'fact', ' title="Some title" id="artifact_1">foo</rillArtifact> Some more text'],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
      ...
      [
        [
          'Some text before <rillArti',
          'fac',
          't title="Some title" id="artifact_1"',
          ' ',
          '>',
          'foo</rillArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        'Before <rillArtifact title="Some title" id="artifact_1">foo</rillArtifact> After',
        {
          output: 'Before  After',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
  ...
  describe('valid artifacts with actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Before <rillArtifact title="Some title" id="artifact_1"><rillAction type="shell">npm install</rillAction></rillArtifact> After',
        {
          output: 'Before  After',
      ...
      [
        'Before <rillArtifact title="Some title" id="artifact_1"><rillAction type="shell">npm install</rillAction><rillAction type="file" filePath="index.js">some content</rillAction></rillArtifact> After',
        {
          output: 'Before  After',
